...
def random_perturbation_loss(embedded, length, loss_fn):
  ...
  noise = tf.random.normal(shape=tf.shape(input=embedded))
...
def virtual_adversarial_loss(logits, embedded, inputs,
  ...
  d = tf.random.normal(shape=tf.shape(input=embedded))
...
def random_perturbation_loss_bidir(embedded, length, loss_fn):
  """Adds noise to embeddings and recomputes classification loss."""
  noise = [tf.random.normal(shape=tf.shape(input=emb)) for emb in embedded]
...
def virtual_adversarial_loss_bidir(logits, embedded, inputs,
  ...
  perturbs = [
      _mask_by_length(tf.random.normal(shape=tf.shape(input=emb)), f_inputs.length)
      for emb in embedded
  ]
  for _ in xrange(FLAGS.num_power_iteration):
    perturbs = [
...
def _kl_divergence_with_logits(q_logits, p_logits, weights):
  ...
  # For softmax regression
    ...
    kl = tf.reduce_sum(
        input_tensor=q * (tf.nn.log_softmax(q_logits) - tf.nn.log_softmax(p_logits)), axis=-1)
  ...
  loss = tf.identity(tf.reduce_sum(input_tensor=weights * kl) / num_labels, name='kl')
  return loss
